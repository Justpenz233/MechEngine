...
void FTriangle::Init(Vector3d _v0, Vector3d _v1, Vector3d _v2) {
    v[0] = _v0;
    v[1] = _v1;
    v[2] = _v2;
}

FTriangle &FTriangle::operator=(const FTriangle &tri) {
    if ( this == &tri )
        return *this;
...
bool FTriangle::IsEqual(FTriangle *tri) {
    if ( this->v[0] == tri->v[0] &&
         this->v[1] == tri->v[1] &&
         this->v[2] == tri->v[2] ) {
        return true;
...
void FTriangle::ComputeCenter() {
    center = (v[0] + v[1] + v[2]) / 3.0f;
}


void FTriangle::ComputeArea() {
    Vector3d normal = (v[1] - v[0]).cross(v[2] - v[0]);
    area = 0.5f * normal.norm();
}

void FTriangle::ComputeNormal() {
...
void FTriangle::CorrectNormal(Vector3d tagtNormal) {
